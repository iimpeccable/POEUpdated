...
public class MessageLogic {
    ...
    if (choice == 1) {  /// Discard Message option
        ...
        Message discardedMessage = new Message(messageID, message, totalMessagesSent + 1, recipient);
        discardedMessage.setStatus("Discarded");
        sentMessages.add(discardedMessage);
        jsonStorage.saveMessageToJson(messageID, recipient, message, "Discarded");
        JOptionPane.showMessageDialog(null, "Message Discarded and archived.");
        SwingUtilities.invokeLater(() -> new MenuScreen("username"));
        return;
    } else if (choice == 2) {  /// Save for Later option
        String messageID = generateMessageID();
        Message storedMessage = new Message(messageID, message, totalMessagesSent + 1, recipient);
        storedMessage.setStatus("Stored");
        sentMessages.add(storedMessage);
        jsonStorage.saveMessageToJson(messageID, recipient, message, "Stored");
    ...
    String messageID = generateMessageID();
    Message newMessage = new Message(messageID, message, totalMessagesSent + 1, recipient);
    ...
    jsonStorage.saveMessageToJson(messageID,recipient, message, status);
...
public ArrayList<Message> getSentMessages() {
    ...
    for (int i = 0; i < sentMessages.size(); i++) {
        ...
        if (msg.generateMessageHash().equalsIgnoreCase(hash)) {
            sentMessages.remove(i);
            JOptionPane.showMessageDialog(null, "Deleted from memory: " + msg.getMessageID());
            return true;
        }
    }
    return false;
